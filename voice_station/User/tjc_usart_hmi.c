...
void send_goods_number(int num_goods)
{
    char str[64];
    sprintf(str, "main.t4.txt=%d\xFF\xFF\xFF", num_goods);  //main.t1
    tjc_send_string(str);
}
...
static void redraw_all_tracks(void)
{
    ...
    for (uint8_t i = 1; i < track_count; i++)
    {
        ...
        screen_x1 = (track_points_y[i-1] + 75)*1.24;
        screen_y1 = 1.2*(track_points_x[i-1]+325);
        screen_x2 = (track_points_y[i] + 75)*1.24;
        screen_y2 =1.2*(track_points_x[i]+325);
    ...
    if (track_count > 0) 
    {
        screen_x1 = (track_points_y[track_count-1] + 75)*1.24;
        screen_y1 = 1.2*(track_points_x[track_count-1]+325);
...
void drone_display_task(void)
{
    ...
    // 直接更新显示，由调用者控制频率
    update_drone_display_with_fire(drone_data.drone_x, drone_data.drone_y, 1);
}
